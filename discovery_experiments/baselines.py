...
class BaselineSpline:
    ...
    Fixed knot count, no adaptive placement.
    """
    ...
    def fit(self, time, values):
        ...
        # linear space between min and max time, excluding start and end
...
class BaselineKernelRidge:
    ...
    def fit(self, time, values):
        ...
        self.model.fit(t, x)
        x_pred = self.model.predict(t)
        residuals = x - x_pred

        # Dual coefficients are the "parameters"
        coeffs = self.model.dual_coef_

        return {
            "model_type": f"krr_rbf_alpha{self.alpha}_gamma{self.gamma}",
            "parameters": coeffs.tolist(),
            "residuals": residuals.tolist(),
            "residual_norm": np.linalg.norm(residuals)
        }
